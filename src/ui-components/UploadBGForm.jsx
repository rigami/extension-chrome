...
function BGCard({name, previewUrl, preview, size, type, format, onRemove, onDone, onPreview, ...other}) {
            ...
            <div className={classes.details}>
                <CardContent className={classes.content}>
                    <Typography component="h5" variant="h5">
                        {type.map(t => locale.global.bg_type[t]).join("/")}
                    </Typography>
                    <Typography variant="subtitle1" color="textSecondary">
                        Файл: {name}
                    </Typography>
                    <Typography variant="subtitle1" color="textSecondary">
                        Размер: {Math.round(size)}мб
                    </Typography>
                    <Typography variant="subtitle1" color="textSecondary">
                        Тип: {format}
                    </Typography>
                    <Tooltip title="Если вы добавляйте пиксель арт, то выключите этот параметр, что бы фон не размывался">
                        <FormControlLabel
                            control={
                                <Switch
        ...
        <Fragment>
            ...
            {dragFiles && dragFiles.length > 0 && (
                <Box className={classes.dragFile} ref={dragRef}>
                    <DropIcon style={{ marginBottom: theme.spacing(2), fontSize: 48 }}/>
                    <Typography variant="h6">
                        {dragFiles.length === 1 ? "Отпустите, чтобы добавить новый фон" : "Отпустите, чтобы добавить новые фоны"}
                    </Typography>
                </Box>
            )}
            <Drawer
